...
__device__ int inline load_data_points(const unsigned int n,float * const RESTRICT ptx, float * const RESTRICT pty, const unsigned int index, const float *const RESTRICT d_dataptx,
...
__device__ void inline load_data_points_given_point_num(const unsigned int my_point_num, float ptx[POINTS_PER_THREAD], float pty[POINTS_PER_THREAD], const unsigned int index,
...
#if POINTS_PER_THREAD == 1
__device__ int inline store_nearest(const unsigned int n, const unsigned int k,const float ptx, const float pty,
        const unsigned int index, volatile const float *const RESTRICT s_centroidx, volatile const float *const RESTRICT s_centroidy)
{
...
__device__ void inline store_nearest(const unsigned int k,const float ptx[POINTS_PER_THREAD], const float pty[POINTS_PER_THREAD],
        volatile const float *const RESTRICT s_centroidx, volatile const float *const RESTRICT s_centroidy,
...
__device__ void inline store_nearest_in_global(const unsigned int n, const unsigned int k,const float ptx,
        const float pty, const unsigned int index, volatile const float *const RESTRICT s_centroidx,
        volatile const float *const RESTRICT s_centroidy, int *const RESTRICT d_clusterno)
{
    // Find and store closest centroid if this thread holds a data-point, else store -1
...
reduce_exp(float *const RESTRICT tmp_sumx, float *const RESTRICT tmp_sumy, int *const RESTRICT tmp_count,
       ...
       volatile int *const RESTRICT s_thrd_k)
{
 ...
 for(int stride = 512; stride>=64; stride=(stride>>1))
 {
   if (BLOCKDIM >= (stride<<1))
   {
     if (tid < stride)
     {
...
__device__ inline void
reduce_exp_nosync(float *const RESTRICT tmp_sumx, float *const RESTRICT tmp_sumy, int *const RESTRICT tmp_count,
       volatile float *const RESTRICT s_thrd_x, volatile float *const RESTRICT s_thrd_y,
       volatile int *const RESTRICT s_thrd_k)
{
 float sumx=(*tmp_sumx);
 ...
 for(int stride = 32; stride>=1; stride=(stride>>1))
 {
   if (BLOCKDIM >= (stride<<1))
   {
       s_thrd_x[tid] = sumx = sumx + s_thrd_x[tid + stride];
...
#if POINTS_PER_THREAD == 1
__device__ int inline reduceThreads(const unsigned int k, float *const RESTRICT sumx, float *const RESTRICT sumy,
                                    volatile float *const RESTRICT s_thrd_x, volatile float *const RESTRICT s_thrd_y,
...
cluster(int n, int k,int *const RESTRICT max_iter, const float *const RESTRICT d_dataptx, const float *const RESTRICT d_datapty,
        ...
        float *const RESTRICT d_sumx, float *const RESTRICT d_sumy, int *const RESTRICT d_count,int *const RESTRICT d_clusterno)//, int *const d_reducedCounts, clock_t *const d_timeVar)//, int *const d_dbgIter)
{
    ...
    for ( numIter = 0; repeat && (numIter < (*max_iter)); ++numIter )
    {
  ...
  #if POINTS_PER_THREAD == 1
          int closest;
          closest=store_nearest(n, k, ptx, pty, index, s_centroidx, s_centroidy);
  #else
          int closest[POINTS_PER_THREAD];
          store_nearest(k, ptx, pty, s_centroidx, s_centroidy, closest, my_point_num);
  #endif
        ...
        count += reduceThreads(k, &sumx, &sumy, s_thrd_x, s_thrd_y, s_thrd_k, ptx, pty, closest);
...
// 3. All the centroids can be accomodated in shared mem. at once.
__global__ void __launch_bounds__(BLOCKDIM, MIN_NUM_BLOCKS_PER_MULTIPROC)
cluster1(int n, int k,int *const RESTRICT max_iter, const float *const RESTRICT d_dataptx, const float *const RESTRICT d_datapty,
         float *const RESTRICT d_centroidx, float *const RESTRICT d_centroidy, int *const RESTRICT d_syncArr,
         float *const RESTRICT d_sumx, float *const RESTRICT d_sumy, int *const RESTRICT d_count,int *const RESTRICT d_clusterno)//, int *const d_reducedCounts, clock_t *const d_timeVar)//, int *const d_dbgIter)
{
    // shared memory
...
__device__ inline void reduceThreads_and_setCentroidVars(const int n, const int k, float *const RESTRICT ptx, float *const RESTRICT pty, float *const RESTRICT s_centroidx,
...
// 3. All the centroids can be accomodated in shared mem. at once.
__global__ void __launch_bounds__(BLOCKDIM, MIN_NUM_BLOCKS_PER_MULTIPROC)
cluster1_load_optimized(int n, int k,int *const RESTRICT max_iter, const float *const RESTRICT d_dataptx, const float *const RESTRICT d_datapty,
                       float *const RESTRICT d_centroidx, float *const RESTRICT d_centroidy, int *const RESTRICT d_syncArr,
