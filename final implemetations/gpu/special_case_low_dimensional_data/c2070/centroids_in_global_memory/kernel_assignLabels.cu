...
__global__ void assign_labels( int n, int k, float *dCentroid, float *dData, int *Index, size_t pitchData)
{
...
#define INVOKE_ASSIGN_LABELS_DEV(i)                                                                 \
  ...
  assign_labels<i><<<dim3(blockDim),dim3(threadDim)>>>( n, k, dCentroid, dData, Index, pitchData);  \
  break;


void callAssignLabels(int n, int k, float *dCentroid, float *dData, int *Index, size_t pitchData, int d, int blockDim,
                      int threadDim)  {
    switch(d)
    {
