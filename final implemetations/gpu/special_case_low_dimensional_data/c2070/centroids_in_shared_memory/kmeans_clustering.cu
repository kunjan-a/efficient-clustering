...
#define INVOKE_ASSIGN_LABELS(i, assign_time)                                            \
  callAssignLabels(n, k, dCentroid, dData_assign, dIndex, pitchData_assign, i, blockDim_assign, threadDim_assign);  \
                        ...
                        int     threadDim_assign)
{
  ...
  float *dCentroid;
  Q(cudaMalloc( (void**) &dCentroid, sizeCentroid*sizeof(float) ));
  ...
  Q( cudaMemcpy( dCentroid, Centroid_colMajor, sizeCentroid*sizeof(float), cudaMemcpyHostToDevice ) );
#else
  Q( cudaMemcpy( dCentroid, Centroid_rowMajor, sizeCentroid*sizeof(float), cudaMemcpyHostToDevice ) );
#endif
