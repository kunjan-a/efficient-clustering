  ...
  if(pointsToProcessByCurrWarp > 0) {
    ...
    for(int pointsIter = 0; pointsIter < pointsToProcessByCurrWarp; pointsIter+=32)  {
      if((dataIndx+indxInWarp) < n){
        isMember = Index[dataIndx+indxInWarp] == reduceFor;
      }else {
        isMember = 0;
      }
      uint membership = __ballot(isMember);
      #pragma unroll
      for(int bit = 0; bit<32 && membership!=0;bit++)  {
        int i = __ffs(membership)-1;  // Find the position of the first (least significant) bit set to 1 in x , where the least significant bit position is 1
        float *Data = (float*)((char*)dData + (dataIndx+i)*pitchData);
        ADD_DATA_POINT(dimPerThread)
        membership = ((membership >> (i+1))<<(i+1));
      }
...
#define INVOKE_REDUCE_DEV(i)   \
  cudaFuncSetCacheConfig(reduceCluster<i>, cudaFuncCachePreferL1);  \
...
#define INVOKE_EXTRNL_REDUCE_DEV(i)  \
  cudaFuncSetCacheConfig(setCluster<i>, cudaFuncCachePreferL1); \
